...
protected:
    ...
    copyData(void) {
        ...
        size_t to = stm32::copy_to_fifo<Width>(m_sourceData, m_packet, byteCount);
        EXPECT_EQ(to, byteCount);

        size_t from = stm32::copy_from_fifo<Width>(m_packet, m_targetData, byteCount);
    ...
    copyPacketLargerThanBuffer(void) {
        ...
        auto to = stm32::copy_to_fifo<Width>(m_sourceData, packet, byteCount);
        EXPECT_EQ(to, std::min(byteCount, std::min(sizeof(m_sourceData), sizeof(packet))));

        auto from = stm32::copy_from_fifo<Width>(packet, targetBuffer, byteCount);
...
TEST_F(FifoCopyTestGeneric, InNullPtr) {
    ...
    auto len = stm32::copy_from_fifo<1, decltype(in)::const_iterator>(nullptr, nullptr, out.begin(), out.end());
    EXPECT_EQ(0, len);
}
...
TEST_F(FifoCopyTestGeneric, InBeginBeyondEnd) {
    ...
    auto len = stm32::copy_from_fifo<1, decltype(in)::const_iterator>(in.end() + 1, in.end(), out.begin(), out.end());
    EXPECT_EQ(0, len);
}
...
TEST_F(FifoCopyTestGeneric, OutNullPtr) {
    ...
    auto len = stm32::copy_from_fifo<1, decltype(in)::const_iterator, decltype(out)::iterator>(in.begin(), in.end(), nullptr, nullptr);
    EXPECT_EQ(0, len);
}
...
TEST_F(FifoCopyTestGeneric, OutBeyondEnd) {
    ...
    auto len = stm32::copy_from_fifo<1, decltype(in)::const_iterator>(in.end(), in.end(), out.end() + 1, out.end());
    EXPECT_EQ(0, len);
}
